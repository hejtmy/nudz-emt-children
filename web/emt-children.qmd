...
df_vr_all %>%
  ...
  gt() %>%
  tab_header(title = "Number of errors in the 3 item difficulty",
      subtitle = "Separated by error type") %>%
...
df_vr_all %>%
  ...
  gt() %>%
  tab_header(title = "Number of times participants made at least one error",
      subtitle = "separated by error type") %>%
    fmt_number(columns = where(is.numeric), decimals = 2)
```
