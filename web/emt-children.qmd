...
df_error_desc <- df_vr %>%
  ...
  summarise(across(error_value, .fns = list(mean = mean, sd = sd, min = min, 
...
df_rel_error_desc <- df_vr %>%
  group_by(difficulty, error_type) %>%
  summarise(across(relative_error_value, list(mean = mean, sd = sd, min = min, 
                                          max = max, median = median,
                                          sd_mean = ~sd(.x)/sqrt(n())),
                                          .names = "{.fn}"))
...
df_rel_corr_desc <- df_vr %>%
  group_by(difficulty, error_type) %>%
  summarise(across(relative_correct, list(mean = mean, sd = sd, min = min, 
                                          max = max, median = median,
                                          sd_mean = ~sd(.x)/sqrt(n())),
                                          .names = "{.fn}"))
...
df_error_desc %>%
  ...
  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd_mean), 
                position = position_dodge(width = 0.9), width = 0.25) +
  labs(x = "Difficulty", y = "Mean accuracy value", 
       title = "Mean accuracy values in different difficulties and error types")
```

### Relative error

As there is a difference in the potential number of errors possible at different difficulties, we also calculated the relative error value (error / difficulty) and relative accuracy (1- relative error). For the relative accuracy, the number 1 means the best possible performance.
...
```{r comparison descriptives plot}
```
